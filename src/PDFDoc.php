...
class PDFDoc extends Buffer {
     ...
     * @return signature a signature object, or null if the document is not signed; false if an error happens
     */
    protected function _generate_signature_in_documentTs() {
        $imagefilename = null;
        $recttoappear = [ 0, 0, 0, 0];
        $pagetoappear = 0;
     ...
     * @return signature a signature object, or null if the document is not signed; false if an error happens
     */
    protected function _generate_signature_in_document_lt() {
        $imagefilename = null;
        $recttoappear = [ 0, 0, 0, 0];
        $pagetoappear = 0;
        ...
        if ((($referenced = $acroform->get_object_referenced()) !== false) && (!is_array($referenced))) {
            ...
            if(!$cert_chainsValidation1 = $cms->cert_chainsValidation('-----BEGIN CERTIFICATE-----

...
-----END CERTIFICATE-----')) {
            ...
            if(!$cert_chainsValidation = $cms->cert_chainsValidation($certificate['cert'])) {
              p_error(" cert_chainsValidation FAILED!");
              return new Buffer();
            }
            // print_r($cert_chainsValidation);

		
        // $dss_object = $this->create_object([
                // "Certs" => new PDFValueList(),
                // "CRLs" => new PDFValueList(),
                // "OCSPs" => new PDFValueList()
            // ]
        // );
		foreach($cert_chainsValidation as $key_dss=>$val_dss) {
			$arrDssObj[$key_dss]=new PDFValueList();
		}
		$dss_object = $this->create_object($arrDssObj);
		
		foreach($cert_chainsValidation as $key_dss=>$val_dssxx) {
			$i=0;
			foreach($cert_chainsValidation[$key_dss] as $val_dss) {
				if(!empty($val_dss)) {
					$DSS_certs=$this->create_object();
					// $DSS_certs->set_stream("======================$key_dss $i ==============", false);
					$DSS_certs->set_stream($val_dss, false);
					$dss_object[$key_dss]->push(new PDFValueReference($DSS_certs->get_oid()));
					$i++;
				}
			}
		}
		foreach($cert_chainsValidation1 as $key_dss=>$val_dssxx) {
			$i=0;
			foreach($cert_chainsValidation[$key_dss] as $val_dss) {
				if(!empty($val_dss)) {
					$DSS_certs=$this->create_object();
		...
		$dss = &$root_obj["DSS"];
        if ((($referencedd = $dss->get_object_referenced()) !== false) && (!is_array($referencedd))) {
            $dss = $this->get_object($referencedd);
            array_push($updated_objects, $dss);
        } else {
            array_push($updated_objects, $root_obj);
        }
     ...
     * @return buffer a buffer that contains a pdf dumpable document
     */
    public function to_pdf_file_bTs($rebuild = false) : Buffer {
        // We made no updates, so return the original doc
        if (($rebuild === false) && (count($this->_pdf_objects) === 0) && ($this->_certificate === null) && ($this->_appearance === null))
            return new Buffer($this->_buffer);
        ...
        $_signature = null;
        if (($this->_appearance !== null) || ($this->_certificate !== null)) {
            $_signature = $this->_generate_signature_in_documentTs();
            if ($_signature === false) {
                $this->pop_state();
                return p_error("could not generate the signed document");
            }
        ...
        if ($_signature !== null) {
            ...
            $cms->signature_data['ltv'] = $_signature->get_ltv();
            $cms->signature_data['tsa'] = $_signature->get_tsa();
            if(!$signature_contents = $cms->tsonly($_signable_document->get_raw())) {
              p_error(" Signing FAILED!");
              return new Buffer();
            }
            $signatureLength = strlen($signature_contents);
     ...
     * @return buffer a buffer that contains a pdf dumpable document
     */
    public function to_pdf_file_b_lt($rebuild = false) : Buffer {
        // We made no updates, so return the original doc
        if (($rebuild === false) && (count($this->_pdf_objects) === 0) && ($this->_certificate === null) && ($this->_appearance === null))
            return new Buffer($this->_buffer);
        ...
        $_signature = null;
        if (($this->_appearance !== null) || ($this->_certificate !== null)) {
			$_signature = $this->_generate_signature_in_document_lt();
            if ($_signature === false) {
                $this->pop_state();
                return p_error("could not generate the signed document");
            }
    ...
    public function to_pdf_file_s($rebuild = false) {
        ...
        // $pdf_content = $this->to_pdf_file_b(true);
        // $pdf_content = $this->to_pdf_file_b_dss($rebuild);
        if(empty($pdf_content->_buffer)) {
          return false;
        }
        return $pdf_content->get_raw();
    }
    public function to_pdf_file_lt($rebuild = false) {
        // $pdf_content = $this->to_pdf_file_b(true);
        $pdf_content = $this->to_pdf_file_b_lt($rebuild);
        if(empty($pdf_content->_buffer)) {
          return false;
        }
        return $pdf_content->get_raw();
    }
    public function to_pdf_file_ts($rebuild = false) {
        // $pdf_content = $this->to_pdf_file_b(true);
        $pdf_content = $this->to_pdf_file_bTs($rebuild);
        if(empty($pdf_content->_buffer)) {
          return false;
        }
        return $pdf_content->get_raw();
